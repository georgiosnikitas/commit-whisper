...
- **A live reachability re-probe inside Settings** — Settings SELECTS a provider; the Doctor reachability probe (6.3) and the analyze-time preflight remain the place reachability is verified. Settings only names the "Ollama must be running" note (selection ≠ reachability). [Source: MENUS.md Settings; 6.3]
