...
- [ ] **Task 1 — Verbosity + color policy in the `ui` module (AC2) [src/shared/ui.ts].** [Source: architecture.md "Behavior Modifiers"; ui.ts]
  ...
  - [ ] `export function resolveLogLevel(input: { verbose?: boolean; quiet?: boolean; env: NodeJS.ProcessEnv }): LogLevel` — pure: `quiet` flag → `"quiet"`; else `verbose` flag → `"verbose"`; else `COMMIT_WHISPER_LOG_LEVEL` (∈ quiet|verbose|normal, trimmed) ; else `"normal"`. (A flag beats the env var.)
...
- **`--config <path>` / `COMMIT_WHISPER_CONFIG`** — the config-file selector is part of Story **6.5** (the `~/.commit-whisper` reader/writer + the config-file resolver layer). 6.4's `--show-config` dumps the CURRENT resolution (config-file layer still `{}` until 6.5). [Source: architecture.md `--config`; epics.md Story 6.5]
...
- **A `COMMIT_WHISPER_LOG_LEVEL`-driven structured logger / log file** — verbosity is a 3-level stderr gate, not a logging framework. [Source: architecture.md "Behavior Modifiers"]
