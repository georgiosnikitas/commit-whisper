...
- **UJ-3. Sofia, who is rusty with git, runs it on her own side project to get better.**
  ...
  - **Path:** runs commit-sage on her own repo; the coaching section, grounded in her actual messages and branching, gives her step-by-step changes.
...
- **Remote provider** — the hosting service for a repo: GitHub, GitLab, or Bitbucket (v1).
...
- **Ownership by area** — which authors dominate which directories/files (top hotspots only, to bound cost). `[ASSUMPTION]`
...
- **Merge vs. rebase tendency** — heuristic on whether history is merge-heavy or linear. `[ASSUMPTION]`
- **Direct-to-default-branch rate** — share of commits landing straight on the default branch vs. via merge. `[ASSUMPTION]`
- **Long-lived branch signal** — branches with long spans between divergence and merge, where reconstructable. `[ASSUMPTION]`
...
- **File survival / age** — how long code persists before being rewritten (high-level). `[ASSUMPTION]`
...
- **Overall hygiene score** — a composite, transparent index rolled up from message quality, commit sizing, and workflow signals; always shown with its components so it is not a black box. `[ASSUMPTION: composite definition to be finalized.]`
...
**Notes:** `[NOTE FOR PM]` The exact composite formula for **overall hygiene score** (F) and several `[ASSUMPTION]`-tagged metrics need a feasibility/definition pass — candidates for the architecture phase. None block the PRD shape.
...
**Consequences (testable):**
- Each run yields a Confidence self-assessment surfaced in the output.
...
- Only Metrics and derived summaries are sent to the LLM — never tokens, never raw source file contents beyond what a Metric description requires. `[ASSUMPTION — confirm no raw diffs are sent.]`
...
- The tool ships as a self-contained executable so a user without a preinstalled Node runtime can run it. `[ASSUMPTION: Node SEA / pkg / nexe; "npx" also acceptable for the dev who has Node.]`
...
**Consequences (testable):**
- Free tier analyzes only up to the capped number of most-recent commits and clearly states the cap in output; all other features remain available within that cap. `[ASSUMPTION: cap value set in §9 / Open Questions.]`
...
- License enforcement fails safe and never transmits repository data as part of licensing. `[ASSUMPTION: licensing mechanism TBD — see Open Questions.]`
...
- Multi-language localization of the Narrative — `[ASSUMPTION: English first.]`
...
- **Performance:** must handle very long histories (the core painkiller) within reasonable time and memory; long runs show progress. `[ASSUMPTION: concrete budgets set in architecture — e.g., target time for a 50k-commit repo.]`
...
- **Portability:** ships as a self-contained executable across major OSes (macOS, Linux, Windows). `[ASSUMPTION]`
...
- **Free** — capped at the N most-recent commits (the funnel; full features within the cap). `[ASSUMPTION: N to be set — see Open Questions.]`
...
- `[OPEN]` Prices are a starting point, not yet validated; the gap between paid tiers is wide (possible future mid-tier).

`[NOTE FOR PM]` **Cannibalization risk.** Because Single-device grants unlimited repositories and remote servers, a small team can buy N× Single-device (e.g., 4 × $2.99 = $11.96) and obtain everything the Unlimited tier offers *except* (a) multiple devices under one license and (b) headless/CI automation (FR-15). The Unlimited tier's marginal value therefore rests almost entirely on CI/automation and centralized multi-device licensing. Decide explicitly whether that justifies the price gap, or whether Single-device should be constrained (e.g., capped repositories/remotes, or no automation) to protect the Unlimited SKU. Linked to Open Q7.
...
`[NOTE FOR PM]` **Measurement model — unresolved tension.** commit-sage is deliberately portal-less, local-first, BYOK, and phones home for nothing (§5, §7). Every metric below therefore describes behavior on the user's machine that the product, as specified, cannot observe. Before these metrics mean anything, choose the measurement model: (a) accept them as proxy/qualitative — inferred from license sales, refund/chargeback rate, review sentiment, and opt-in surveys — and state that here; or (b) introduce explicit opt-in, anonymized telemetry and accept the positioning cost. Do not leave measurement implied.
...
- **SM-3: Conversion.** Free-tier users with long histories / many repos upgrade to a paid tier. Validates FR-16. `[OPEN: target rate TBD.]`
...
1. **Free-tier cap (N commits):** what value balances "proves value" against "creates upgrade pressure"? (Affects FR-3, FR-16, §9.)
2. **License enforcement mechanism:** how is single-device vs. unlimited enforced, perpetually and offline, without transmitting repo data? (Affects FR-16.)
...
- §4.6 FR-14 — self-contained executable via Node SEA / pkg / nexe.
- §4.6 FR-15 — exit-code semantics (unhealthy ≠ failure).
- §4.6 FR-16 / §9 — Free-tier cap value; licensing mechanism.
- §7 — performance budgets; cross-OS portability.
- §6.2 — English-first Narrative (localization deferred).
