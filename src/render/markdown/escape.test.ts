...
describe("escapeCell", () => {
  it("neutralizes the structure-breaking + raw-HTML characters", () => {
    expect(escapeCell("a|b")).toBe(String.raw`a\|b`); // a table-cell breaker
    expect(escapeCell("*x*")).toBe(String.raw`\*x\*`); // emphasis
    expect(escapeCell("`code`")).toBe("\\`code\\`"); // a code span
    expect(escapeCell("[link](x)")).toBe(String.raw`\[link\](x)`); // link syntax
    expect(escapeCell("<script>")).toBe("&lt;script&gt;"); // raw HTML
  });

  it("escapes backslash first so later escapes are not double-escaped", () => {
    expect(escapeCell(String.raw`a\b`)).toBe(String.raw`a\\b`);
  });
  ...
  it("a hostile file path cannot break a table or inject HTML", () => {
    ...
    expect(out).toContain(String.raw`\|evil`); // the pipe is escaped (cannot break a table cell)
